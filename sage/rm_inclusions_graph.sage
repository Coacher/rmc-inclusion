...
if ("--help" in sys.argv) or ("-h" in sys.argv):
    print("This program will contruct and output inclusion tree for Rad*Ms.")
...
output_name = "rm_inclusion_tree_{p}-{l}-{lamb}.png".format( \
        p = p, l = l, lamb = lamb)
...
def gen_elems():
    ...
    for j in range(0, numofMs):
        try:
            i = Ms.index(RMs[j])
            elems.update( {Ms[i]: " "*50 + "M_{pi}({m},{i})".format(pi = pi, m = m, i = i) \
                + " = " + "Rad*M_{pi}({m},{j})".format(pi = pi, m = m, j = j)} )
        except:
            elems.update( {RMs[j]: " "*20 + "Rad*M_{pi}({m},{j})".format(pi = pi, m = m, j = j)} )
    return elems
